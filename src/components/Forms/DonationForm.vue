...
<template>
  <Dialog :open="open" @update:open="emit('update:open', $event)">
    <DialogContent class="sm:max-w-[500px] h-[85vh] flex flex-col p-0">
      ...
      <ScrollArea class="flex-1 px-6">
        <!-- 快速選擇物資（如果有提供需求清單） -->
        <Card
          v-if="supplyItems && supplyItems.length > 0"
          class="p-4 bg-gray-50 mt-4"
        >
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4 py-4 px-1">
          ...
          <div class="space-y-2">
            ...
            />
            <p v-if="errors.itemName" class="text-sm text-red-500">
              {{ errors.itemName }}
            </p>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div class="space-y-2">
              ...
              />
              <p v-if="errors.unit" class="text-sm text-red-500">
                {{ errors.unit }}
              </p>
            </div>
          </div>
